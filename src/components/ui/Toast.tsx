// Importing React and necessary Radix Toast primitives
import * as React from "react"
import * as ToastPrimitives from "@radix-ui/react-toast"

// Utility to create style variants (e.g., default, destructive)
...
// --- Toast Viewport (Position of Toasts) ---
...
// --- Variant styles for Toast (e.g., default, destructive) ---
const toastVariants = cva(
    ...
    "data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border bg-background text-foreground", // Normal theme
        destructive:
          "destructive group border-destructive bg-destructive text-destructive-foreground", // Error/danger theme
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, variant, ...props }, ref) => {
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={cn(toastVariants({ variant }), className)} // Apply chosen variant class
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ...
    toast-close="" // Required for Radix to recognize it
    {...props}
  >
    <X className="h-4 w-4" /> {/* Lucide close icon */}
  </ToastPrimitives.Close>
))
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn("text-sm font-semibold", className)} // Style the title
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn("text-sm opacity-90", className)} // Subdued text
    {...props}
  />
))
