...
type CarouselProps = {
  ...
  setApi?: (api: CarouselApi) => void; // Callback to get API instance
};
...
type CarouselContextProps = {
  ...
  canScrollPrev: boolean; // Can scroll back?
  canScrollNext: boolean; // Can scroll forward?
} & CarouselProps;
...
>(
  (
    ...
    const [carouselRef, api] = useEmblaCarousel(
      {
        ...opts,
        axis: orientation === "horizontal" ? "x" : "y", // Set scroll axis
      },
      plugins
    );

    // State for navigation availability
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn(
        "min-w-0 shrink-0 grow-0 basis-full", // Each item takes full width/height
        orientation === "horizontal" ? "pl-4" : "pt-4", // Spacing based on orientation
        className
      )}
      {...props}
    />
